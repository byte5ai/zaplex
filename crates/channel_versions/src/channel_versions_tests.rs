...
// openWarp (OSS) uses simplified tags of the form vYYYY.MM.DD.N. The following tests ensure that:
// 1. This format can be parsed by ParsedVersion (otherwise is_current_version_ahead_of_latest_version
//    would always return Err, wrongly steering the user toward "upgrading" to a rolled-back version).
// 2. The ordering comparison is monotonic over the (major=0, date, patch) triple.
#[test]
...
fn test_oss_version_without_patch_parses() {
    // Early OSS tags may not have the 4th segment (the sequence number).
...
fn test_oss_version_rollback_detected() {
    // The remote was rolled back: the newly published release tag is older than the current local
    // version, and is_current_version_ahead_of_latest_version should recognize this as true, so the
    // rolled-back version is not wrongly presented as an "upgrade".
