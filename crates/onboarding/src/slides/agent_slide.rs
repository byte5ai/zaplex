...
impl AgentSlide {
    ...
    fn render_disable_oz_section(
        ...
        let label = Text::new(
            localized("onboarding-agent-disable-warp-agent", "Disable Zaplex Agent"),
            appearance.ui_font_family(),
            14.0,
        )
        .with_color(internal_colors::text_sub(theme, background_for_text))
    ...
    fn render_bottom_nav(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let step_index = 2;
        let step_count = if warp_core::features::FeatureFlag::ZaplexNewSettingsModes.is_enabled()
        {
            5
        } else {
            4
