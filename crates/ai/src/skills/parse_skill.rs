...
pub struct ParsedSkill {
    ...
    /// The provider of the skill (Agents, Claude, Codex, or Zaplex), determined from the path.
...
impl ParsedSkill {
    /// Returns true if this skill is bundled with Zaplex (not a user-editable file).
...
/// skill provider directory. Bundled skills are always assigned `SkillProvider::Zaplex`
...
pub fn parse_bundled_skill(path: &Path) -> Result<ParsedSkill> {
    parse_skill_internal(path, SkillProvider::Zaplex, SkillScope::Bundled)
}
