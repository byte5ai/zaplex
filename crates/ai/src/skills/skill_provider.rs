...
//!
//! This module defines the supported skill providers (i.e. Agents, Claude, Codex, Zaplex) and their
...
/// Represents a skill provider/origin (Agents, Claude, Codex, or Zaplex).
#[derive(
    Debug,
    Clone,
    Copy,
...
pub enum SkillScope {
    ...
    #[default]
    Project,
    /// Bundled skills distributed with Zaplex.
    Bundled,
}
...
pub fn home_skills_path(provider: SkillProvider) -> Option<PathBuf> {
    if provider == SkillProvider::Zaplex {
...
mod tests {
    ...
    fn warp_home_skills_path_uses_warp_home_path() {
        assert_eq!(
            home_skills_path(SkillProvider::Zaplex),
    ...
    fn warp_home_skill_path_uses_warp_provider() {
        ...
        assert_eq!(get_provider_for_path(&path), Some(SkillProvider::Zaplex));
    }
}
