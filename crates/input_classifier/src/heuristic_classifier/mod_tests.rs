...
fn test_cjk_input_detection() {
    futures::executor::block_on(async move {
        ...
        // Default triggered from Shell mode (stricter scenario, verify CJK can still switch to AI).
        let context = Context {
            current_input_type: InputType::Shell,
            is_agent_follow_up: false,
        };

        // Single CJK character also detected as AI (default logic would classify as Shell for token count < 2).
        ...
        // Mixed English-CJK, any CJK triggers AI.
        ...
        // CJK punctuation (full-width comma, period, question mark) also triggers.
        ...
        // Pure English shell commands are not affected.
