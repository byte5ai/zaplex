...
impl LocalRepoMetadataModel {
    ...
    fn add_repository_internal(
        ...
        // Register this path with the watcher if we have one. Skip the home directory and its
        // ancestors to avoid recursively watching unrelated user data; they can still be listed.
        #[cfg(feature = "local_fs")]
        {
    ...
    pub fn remove_repository(
        ...
        if self.repositories.remove(repo_path).is_some() {
            // Mirror the registration guard: unsafe roots were never registered.
            #[cfg(feature = "local_fs")]
            {
                if let Some(ref watcher) = self.watcher {
