...
//!
//! Design principle: each input field always displays the currently saved value and can be edited directly
//! (including clearing), submitted via the "Save" button next to it. Password fields use `is_password: true`
...
/// Maximum width of the input area (editor + two buttons), aligned with the slot constraint to the right of field labels.
const INPUT_AREA_MAX_WIDTH: f32 = 420.0;

const BUTTON_PADDING: f32 = 6.0;

/// Read system proxy from environment variables (cross-platform minimum set): returns (https_proxy, http_proxy, no_proxy).
/// Deeper reading from Windows WinINET / macOS SCDynamicStore is deferred to a future PR.
...
pub enum NetworkPageAction {
    /// Dropdown selected a ProxyMode item, persist to settings.
...
/// The probing method chosen for this test. Used by result messages to select appropriate descriptions.
#[derive(Debug, Clone, Copy)]
enum TestKind {
    /// TCP probing of proxy host:port (validates proxy connectivity, suitable for corporate networks / VPN proxies).
    /// Used for Custom mode and System mode when system proxy can be detected from environment variables.
    Tcp,
    /// HTTP GET probing of a public URL. Only used for Off mode or as a fallback for System mode
    /// when system proxy detection fails.
    Http,
}

/// Test result (returned from async task to main thread's handle_action).
#[derive(Debug, Clone)]
...
/// Current state of the connection test.
...
pub struct NetworkPageView {
    ...
    /// Editors for each field (password field has `is_password` mask enabled).
    ...
    /// Mouse state for the two buttons (save + clear) corresponding to each field.
    ...
    /// Mouse state and state for the test connection button.
...
impl NetworkPageView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        // Subscribe to settings / credentials changes — when any field or mode is changed externally,
        // sync the latest values back to each editor's buffer and update the dropdown selection.
        ...
        // Initial sync to display current saved values in dropdown and editors.
        Self::sync_all_from_settings(&mut me, ctx);
        Self::sync_password_from_credentials(&mut me, ctx);
        me
    }

    /// Sync current NetworkSettings values into the dropdown and three non-password editors.
    fn sync_all_from_settings(me: &mut Self, ctx: &mut ViewContext<Self>) {
        ...
        // Dropdown selection follows mode.
        ...
        // Editor buffers follow setting values; InteractionState changes based on mode.
        ...
        // Password also follows mode for interaction state (buffer is synced separately via ProxyCredentials subscription).
    ...
    /// Sync current password into the password editor (managed separately by ProxyCredentials).
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            NetworkPageAction::TestConnection => {
                ...
                // Decide test strategy based on current mode:
                //   Custom → TCP probing of proxy host:port (proxy connectivity, independent of outbound, suitable for corporate proxies)
                //   System / Off → HTTP GET probing of public URL (outbound connectivity)
...
/// Select probing method based on mode, spawn to background, and return results to main thread via action.
fn spawn_test_connection(
    ...
    match mode {
        ProxyMode::Custom => {
            // User-provided proxy: parse and then TCP probe host:port.
        ...
        ProxyMode::System => {
            // Prioritize reading system proxy from environment variables (cross-platform minimum set).
            // If found, use TCP probing; if not found (macOS SCDynamicStore / Windows WinINET are only used
            // internally by reqwest), fall back to HTTP probing of public network.
        ...
        ProxyMode::Off => {
            // Off mode has no proxy to test; instead, test if direct outbound connection works.
            spawn_http_probe(timeout, ctx);
        }
    }
}

/// Extract TCP probing logic into a helper function for reuse between Custom and System paths.
fn spawn_tcp_probe(
    host: String,
    port: u16,
    timeout: Duration,
...
/// HTTP probing logic (uses reqwest's global proxy settings). Only used for Off or System fallback scenarios.
...
/// Build a single-line EditorView with optional password masking.
...
/// Write current value into editor buffer and toggle InteractionState based on `editable`.
/// Note: `set_buffer_text` resets the cursor, so it should not be called while user is focused on editing —
/// this function is only used when settings are changed externally.
fn set_editor_text_and_state(
    ...
    editor.update(ctx, |editor, ctx| {
        // If buffer already equals target value, skip set to avoid unnecessary cursor reset.
...
impl SettingsWidget for NetworkPageWidget {
    ...
    fn render(
        ...
        // Note: The `_app` passed to SettingsWidget::render is the AppContext at render time; we need it to read the current mode.
        // We keep the parameter name as is for now to avoid widespread changes; we'll use `_app` directly below.
        ...
        // Field rendering helper: one editor + save button + clear button, aligned to uniform width.
        let render_field = |label: String,
         ...
         -> Box<dyn Element> {
            ...
            // Note: Do not put `margin` in button's `UiComponentStyles`.
            // `WrappableText::build()` (inside `Span::new(text, styles).build()`)
            // applies the same `styles.margin` to the label container, causing the button's
            // internal label to also be pushed left the same distance, appearing as "text misaligned right".
            // Instead, use an outer Container to set horizontal spacing between button and editor/adjacent buttons.
            ...
            let input_area = ConstrainedBox::new(
                Flex::row()
                    ...
                    .with_child(
                        // Editor takes remaining space, wrapped in a constrained-width container
                        // (prevents internal flex from breaking under unconstrained layout).
        ...
        // 6. Test Connection — same style as save button above.
        ...
        // Idle hint text should match current mode: Custom tests proxy connectivity, System/Off tests outbound connectivity.
        ...
        // Wrap outer layer with Align(left) to prevent parent Flex from stretching button height on cross-axis;
        // inner Flex::row uses MainAxisSize::Min, taking only the necessary width.
        content.add_child(
            Container::new(
                Align::new(
                    Flex::row()
