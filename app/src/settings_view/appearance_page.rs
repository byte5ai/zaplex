...
use crate::settings::{
    ...
    InputModeSettings, InputModeState, MARKDOWN_HEADING_SCALE_MAX, MARKDOWN_HEADING_SCALE_MIN,
    MarkdownHeadingH1Scale, MarkdownHeadingH2Scale, MarkdownHeadingH3Scale, MarkdownHeadingH4Scale,
    MarkdownHeadingH5Scale, MarkdownHeadingH6Scale, MonospaceFontName, PaneSettings,
    ShouldDimInactivePanes, ThemeSettings, UI_FONT_SIZE_MAX, UI_FONT_SIZE_MIN, UiFontName,
    UseSystemTheme, active_theme_kind,
...
pub fn init_actions_from_parent_view<T: Action + Clone>(
    ...
    // Add command palette entry for toggling between Zaplex and Classic input modes
    app.register_fixed_bindings(vec![
        FixedBinding::empty(
            "Toggle Input Mode (Zaplex/Classic)".to_string(),
            builder(SettingsAction::AppearancePageToggle(
...
pub struct AppearanceSettingsPageView {
    ...
    /// selected Zaplex prompt configuration.
...
impl AppearanceSettingsPageView {
    ...
    /// Handles edit events from the markdown heading font-size scale editor.
    ...
    fn set_markdown_heading_scale(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        // Fallback: for any field that failed to parse, or parsed successfully but whose
        // clamped value equals the current value (set_value emits no event), the editor
        // must snap back to the setting's real value so it never stays on invalid or
        // out-of-range user input.
        self.refresh_markdown_heading_scale_editors(ctx);
    }

    /// Resets the Markdown heading font-size scale factors to their defaults.
...
impl SettingsWidget for CreateCustomThemeWidget {
    ...
    fn render(
        ...
        appearance
            ...
            .with_centered_text_label(crate::t!("theme-editor-open"))
            .build()
            .on_click(|ctx, _, _| {
                ctx.dispatch_typed_action(crate::WorkspaceAction::OpenThemeCreatorModal)
            })
            .finish()
    }
}
...
/// Settings widget for the Markdown heading font-size scale factors.
impl SettingsWidget for MarkdownHeadingScaleWidget {
    ...
    fn render(
        ...
        // Title row: title and reset button on the same line.
