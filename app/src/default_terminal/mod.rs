...
mod non_mac {
    ...
    /// Sets Zaplex as the default terminal
...
pub struct DefaultTerminal {
    /// Whether the OS will treat Zaplex as the default app for scripts/executables.
...
impl DefaultTerminal {
    ...
    /// This is an OS-level setting. Unlike most other settings, where Zaplex is the source-of-truth
    /// for the value of the setting, it can be changed outside of Zaplex. We monitor if it gets
    /// changed externally by checking when Zaplex is focused.
    ...
    /// This is a one-way operation. Once we set the default terminal to Zaplex, we can't really
    ...
    pub fn make_warp_default(&mut self, ctx: &mut ModelContext<Self>) {
        if let Err(e) = set_warp_as_default_terminal() {
            log::error!("Error setting Zaplex as default terminal: {e:#}");
        } else {
