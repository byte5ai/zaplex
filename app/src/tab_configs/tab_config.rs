...
///
/// `followup_command` (e.g. an agent fork command, fork/worktree design §3)
/// is appended verbatim as one extra command after the `cd` into the worktree.
#[cfg(feature = "local_fs")]
pub(crate) fn build_worktree_config_toml(
    ...
    let commands_with_followup = |mut commands: Vec<Value>| {
        if let Some(followup) = followup_command {
            commands.push(Value::String(followup.to_string()));
        }
        commands
    };
    ...
    if worktree_branch_name.is_some() {
        ...
        pane.insert(
            "commands".into(),
            Value::Array(commands_with_followup(vec![
                Value::String(format!(
