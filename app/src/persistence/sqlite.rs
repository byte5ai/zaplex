...
/// Prewarm SQLite connection + execute migration in a background thread. Should be called
/// before `app_builder.run` is invoked, so that SQLite initialization can proceed concurrently
/// with subsequent winit / wgpu initialization, saving ~70–90ms on cold startup.
///
/// If this function is not called, `initialize` will fall back to the synchronous path (original behavior).
...
/// Returns `None` if prewarming was never initiated; caller falls back to synchronous path.
...
unsafe fn init_logging() {
    ...
    extern "C-unwind" fn log_callback(_data: *mut c_void, err_code: c_int, msg: *const c_char) {
        ...
        // Local logging path shouldn't panic, but still avoid unwinding across FFI boundary.
        let _ = panic::catch_unwind(|| {
            // openWarp only writes to local logs, error codes/descriptions still retain diagnostic value.
            log::log!(
                level,
                "SQLite error {} ({}): {}",
                err_code,
                ...
                for event in events {
                    match event {
                        ...
                        ModelEvent::Terminate => {
                            // Before exiting, actively checkpoint all WAL back to the main database and clear it,
                            // to avoid sqlite3_open triggering "WAL mode database file was recovered" on next startup.
                            if let Err(e) =
...
/// Helper to delete local stored object identified by `sync_id`. If a valid object metadata row is found,
/// calls `delete_object_fn` to delete the actual object.
