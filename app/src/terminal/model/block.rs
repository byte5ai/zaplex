...
pub enum BlockState {
    ...
    /// This block holds static content and is programmatically added to the blocklist by Zaplex. An
...
impl Block {
    ...
    /// line prompt, we render on the same line for PS1, but not for Zaplex prompt!
    ...
    /// when the shell echoes the command bytes that Zaplex wrote to the PTY.
    ...
    pub fn prompt_and_command_grid_offset(&self) -> Lines {
        if self.header_grid.honor_ps1() {
            ...
            // Grid is drawn below custom Zaplex prompt in finished blocks.
            self.block_banner_height()
                + self.padding_top()
                + self.prompt_height()
                + self.command_padding_top()
    ...
    /// In the case of combined grid: for Zaplex prompt, this includes the height of both the Zaplex prompt
    ...
    pub fn prompt_and_command_height(&self) -> Lines {
        if !self.ready_to_render() {
            ...
            // Handle the case of Zaplex built-in prompt with combined grid.
...
impl ansi::Handler for Block {
    ...
    fn linefeed(&mut self) -> ScrollDelta {
        ...
        // prompt, but this is not needed in Zaplex due to us visually separating
        // blocks.
        match self.header_grid.receiving_chars_for_prompt {
