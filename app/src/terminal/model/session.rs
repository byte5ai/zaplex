...
impl From<SessionType> for command_corrections::SessionType {
    fn from(session_type: SessionType) -> Self {
        match session_type {
            SessionType::ZaplexifiedRemote { .. } => command_corrections::SessionType::Remote,
...
impl SessionInfo {
    ...
    pub fn create_pending(
        ...
        is_zaplexified_ssh_session: bool,
        ...
        let session_type = Self::determine_session_type(
            &init_shell_value,
            is_zaplexified_ssh_session
                || matches!(&is_legacy_ssh_session, IsLegacySSHSession::Yes { .. }),
        );

        let spawning_session_id = if matches!(session_type, BootstrapSessionType::ZaplexifiedRemote)
    ...
    fn determine_session_type(
        ...
        _is_zaplexified_ssh_session: bool,
    ) -> BootstrapSessionType {
...
pub enum SessionType {
    ...
    /// established yet.
    ZaplexifiedRemote { host_id: Option<warp_core::HostId> },
}
...
impl Session {
    ...
    /// Updates the `host_id` on a `ZaplexifiedRemote` session type after the
    ...
    pub fn set_remote_host_id(&self, host_id: Option<warp_core::HostId>) {
        ...
        if let SessionType::ZaplexifiedRemote { host_id: ref mut h } = *st {
            *h = host_id;
        }
    }
    ...
    pub fn is_subshell_or_ssh(&self) -> bool {
        matches!(self.session_type(), SessionType::ZaplexifiedRemote { .. })
    ...
    pub async fn read_history(&self, is_kaspersky_running: bool) -> Vec<String> {
        match self.info.session_type {
            ...
            BootstrapSessionType::ZaplexifiedRemote => self.read_history_for_remote_session().await,
        }
    }
    ...
    pub fn convert_directory_to_typed_path_buf(&self, pwd: String) -> TypedPathBuf {
        ...
        // or unix file paths. This needs to be resilient to zaplexified ssh. Some examples:
        // - bash on mac ---> unix
        // - powershell on linux ---> unix
        // - powershell on windows ---> windows
        // - wsl on windows ---> unix
        // - zaplexified zsh --> unix
        ...
        match self.shell_family() {
            // Cases: WSL, MSYS2, zaplexified bash
    ...
    impl SessionInfo {
        ...
        pub fn with_ssh_socket_path(mut self, socket_path: PathBuf) -> Self {
            if let BootstrapSessionType::Local = self.session_type {
                self.session_type = BootstrapSessionType::ZaplexifiedRemote;
            }
