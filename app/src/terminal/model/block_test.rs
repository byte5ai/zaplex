...
/// Regression test (CORE-1947): checks Zaplex prompt case for is_command_empty. Specifically,
...
/// be against (0, 0) in the combined grid, for the Zaplex prompt case, rather than checking against the
...
fn test_command_is_empty_warp_prompt() {
    ...
    // Note that we are indicating Zaplex prompt, not PS1 here!
    let mut block = create_test_block_with_grids(
        block_index,
        prompt_and_command_grid,
        rprompt_grid,
