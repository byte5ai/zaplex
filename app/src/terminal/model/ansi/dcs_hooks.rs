...
/// Indicates that the following JSON-encoded message is hex-encoded for Zaplex's lifecycle hooks.
...
/// Indicates that the following JSON-encoded message is unencoded for Zaplex's lifecycle hooks.
/// In DCS, it is used as the final char in the DCS start sequence.
/// In OSC, it is used as the first parameter.
pub(super) const UNENCODED_JSON_MARKER: char = 'f';

/// Indicates that the following message is a ANSI-C quoted message for receiving Zaplex's lifecycle
/// hooks via key-value pairs.
/// In OSC< it is used as the first parameter.
pub(super) const UNENCODED_KV_MARKER: char = 'k';

/// Enum representing all possible JSON payloads for Zaplex's DCS's.
...
impl DProtoHook {
    pub fn name(&self) -> &'static str {
        match self {
            ...
            DProtoHook::RemoteZaplexificationIsUnavailable { .. } => {
                "RemoteZaplexificationIsUnavailable"
            }
...
/// The reason that zaplexification was not available when the user tried
/// to zaplexify.
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all(serialize = "snake_case"))]
pub enum ZaplexificationUnavailableReason {
    TmuxFailed,
...
/// Received from the pty after the shell has finished executing Zaplex's
/// bootstrap script.
...
/// Received from the pty when Zaplex's SSH wrapper is executed, prior to
...
/// created; if the session is for a subshell, this triggers Zaplex's bootstrap process.
...
/// The binding is triggered when Zaplex writes the `ESC-i` escape sequence to the pty.
/// Zaplex usually does this after a block completes, to collect any typeahead
...
/// `logout`, Ctrl-D on an empty prompt, etc.). Lets the Zaplex client drop
