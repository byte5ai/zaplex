...
pub trait Handler {
    ...
    /// Callback for the Zaplex CommandFinished hook.
    ...
    /// Callback for the Zaplex precmd hook.
    fn precmd(&mut self, _data: PrecmdValue) {}

    /// Callback for the Zaplex preexec hook.
    fn preexec(&mut self, _data: PreexecValue) {}

    /// Callback for the Zaplex bootstrapped hook - called once when the shell is
    /// bootstrapped
    fn bootstrapped(&mut self, _data: BootstrappedValue) {}

    /// Callback for the Zaplex pre-interactive SSH session hook - called once
    ...
    /// Callback for the Zaplex ssh hook - called once after successfully connecting to
    /// an SSH server
    ...
    /// Callback for the Zaplex exit-shell hook — emitted by the remote shell right
    /// before it exits. Gives the Zaplex client a chance to drop per-session
    ...
    /// input buffer (the reporting is itself triggered by Zaplex).
    ...
    /// created. If the session is for a subshell, this should triggers Zaplex's bootstrap process.
    ...
    /// Callback emitted from the zaplexify_ssh_session script if it's discovered
    /// that we can't zaplexify the remote session.
    fn remote_zaplexification_is_unavailable(&mut self, _data: ZaplexificationUnavailableReason) {}
    ...
    /// These allow external programs to trigger notifications in Zaplex.
