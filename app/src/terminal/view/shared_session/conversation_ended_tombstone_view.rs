...
struct TombstoneDisplayData {
    ...
    /// Record task execution harness when available from local data.
    #[cfg(not(target_family = "wasm"))]
...
impl TombstoneDisplayData {
    ...
    /// Supplement display data with AmbientAgentTask data.
    #[cfg(not(target_family = "wasm"))]
...
impl ConversationEndedTombstoneView {
        ...
        ctx.subscribe_to_view(
            ...
            |_, _, event, ctx| match event {
                ArtifactButtonsRowEvent::OpenPlan { document_uid } => {
                    // openWarp localization: in shared session tombstone, also use local
                    // AIDocument pane, no longer depend on cloud notebook_uid.
    ...
    fn render_action_buttons(
        ...
        #[cfg(not(target_family = "wasm"))]
        {
            ...
            // forked into a local Zaplex conversation. Unknown harness (None) is
