...
use crate::terminal::zaplexify::render;
...
pub enum SshInstallTmuxBlockEvent {
    InstallTmuxAndZaplexify(TmuxInstallMethod),
...
impl SshInstallTmuxBlock {
    ...
    pub fn emit_install_tmux(
        ...
        ctx.emit(SshInstallTmuxBlockEvent::InstallTmuxAndZaplexify(
            install_method,
        ));
        ctx.notify()
    }
...
impl SshInstallTmuxBlock {
    ...
    fn render_title_ui(
        ...
        let right_hand_size = is_awaiting_action
            .then(|| {
                render::render_never_zaplexify_ssh_link(
                    &self.ssh_host,
                    app,
                    appearance,
...
impl View for SshInstallTmuxBlock {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let explanation = if self.outdated_version {
            "In order to Zaplexify your SSH session, a more recent version of tmux (>=3.0) must be installed. "
        } else {
...
pub fn install_tmux_script(system: &SystemDetails, app: &AppContext) -> Option<String> {
    ...
    let asset_source = match (
        ...
        ("Linux", _, "bash" | "zsh") => {
            bundled_asset!("ssh/bash_zsh/install_tmux_and_zaplexify_linux.sh")
        }
        ("Linux", _, "fish") => {
            bundled_asset!("ssh/fish/install_tmux_and_zaplexify_linux.sh")
        }
...
pub fn install_root_tmux_script(
    ...
    let asset_source = match (
        ...
        ("Linux", "apt", "bash" | "zsh") => {
            bundled_asset!("ssh/bash_zsh/root/install_tmux_and_zaplexify_apt.sh")
        }
        ("Linux", "dnf", "bash" | "zsh") => {
            bundled_asset!("ssh/bash_zsh/root/install_tmux_and_zaplexify_dnf.sh")
        }
        ("Linux", "pacman", "bash" | "zsh") => {
            bundled_asset!("ssh/bash_zsh/root/install_tmux_and_zaplexify_pacman.sh")
        }
        ("Linux", "yum", "bash" | "zsh") => {
            bundled_asset!("ssh/bash_zsh/root/install_tmux_and_zaplexify_yum.sh")
        }
        ("Linux", "zypper", "bash" | "zsh") => {
            bundled_asset!("ssh/bash_zsh/root/install_tmux_and_zaplexify_zypper.sh")
        }
        _ => return None,
    };
