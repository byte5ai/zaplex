...
pub fn should_render_ps1_prompt(terminal_model: &TerminalModel, app: &AppContext) -> bool {
    ...
    // if the sharer is using PS1 and the viewer is not (using Zaplex prompt in non-SLP mode).
...
pub fn should_render_prompt_on_same_line(
    ...
    // 2. The user has the same line prompt setting enabled for their Zaplex prompt.
