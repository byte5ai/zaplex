...
//!
//! This module owns everything specific to running a Zaplex shell inside a
...
///
/// Lives under the Zaplex per-user cache directory rather than `/tmp` so:
...
/// Resolves the absolute path to the `sbx` CLI binary using the Zaplex
/// process's `PATH`.
///
/// Zaplex's process `PATH` is minimal and often misses user-shell-installed
/// tools (e.g. homebrew on Apple Silicon when Zaplex is launched from Finder,
...
///
/// Each instance carries a unique `sandbox_id` so multiple Zaplex panes can run
...
impl DockerSandboxShellStarter {
    ...
    /// Host directory where Zaplex writes this sandbox's bash init script.
