...
impl UpdateManager {
    ...
    fn move_object_to_drive(
        ...
        // Local copying of workflow enums to target owner still needs to happen — `update_object` /
        // `create_object` are both local stubs; this call is a purely local model operation.
    ...
    /// Parameter `revision_ts` is kept for interface stability; it is ignored in this local branch
    /// (will be cleaned up uniformly during Phase 2d-4b renaming).
    ...
    pub fn empty_trash(&mut self, space: Space, ctx: &mut ModelContext<Self>) {
        ...
        // (which already handles in-memory + SQLite dual deletion + action cleanup).
