...
///
/// Maps one-to-one with `http_client::ProxyMode` / `websocket::ProxyMode`; defined separately
/// to decouple configuration layer from infrastructure layer, and because this type must implement
...
#[schemars(
    description = "HTTP proxy mode: off completely disables (default); system uses system/environment; custom uses explicit URL.",
    rename_all = "snake_case"
)]
pub enum ProxyMode {
    /// Completely disable proxy, including environment variables. Default; avoids unexpected system proxies detected by reqwest interfering with local calls.
    #[default]
    Off,
    /// Follow system proxy / environment variables (reqwest default behavior).
    System,
    /// Use user-specified URL.
    Custom,
}
...
impl ProxyMode {
    ...
    /// Convert to `websocket::ProxyMode` (independent mirror, see top comment in websocket/proxy.rs).
