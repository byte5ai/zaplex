...
lazy_static! {
    ...
    static ref ZAPLEX_SECRET_DELIMITER: &'static str = "/warp-secret-delimeter/";
    static ref LASTPASS_LIST_SECRETS_COMMAND: Vec<String> = {
        vec![
            "lpass".to_owned(),
            "ls".to_owned(),
            format!("--format=%an{}%ai", *ZAPLEX_SECRET_DELIMITER),
        ]
    };
}
...
fn parse_lastpass_secrets(output: &str) -> anyhow::Result<Vec<ExternalSecret>> {
    let parsed_output: Vec<ExternalSecret> = output
        .lines()
        .filter_map(|line| {
            let parts = line.split(*ZAPLEX_SECRET_DELIMITER).collect_vec();
