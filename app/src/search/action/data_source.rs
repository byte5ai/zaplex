...
impl CommandBindingDataSource {
    ...
    pub fn new(binding_source: ModelHandle<BindingSource>, ctx: &mut ModelContext<Self>) -> Self {
        ...
        // Fuzzy matching also lets an English keyword hit binding.name via subsequence matching (see the search implementation below).
    ...
    fn search(&self, search_term: &str) -> anyhow::Result<Vec<QueryResult<SearcherAction>>> {
        Ok(self
            ...
            .filter_map(move |binding| {
                ...
                .map(|mut result| {
                    // Highlight rendering targets the description; indices that land in the
                    // appended binding.name segment are out of bounds and would draw in the wrong place, so trim them here.
                    result
                        .matched_indices
