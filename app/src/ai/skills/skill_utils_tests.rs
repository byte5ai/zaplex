...
fn test_skill_path_from_file_path_warp_home_skill() {
    let Some(warp_home_skills_dir) = warp_core::paths::warp_home_skills_dir() else {
        eprintln!("Skipping test: Zaplex home skills directory not available");
        return;
    };
...
fn test_unique_skills_keeps_same_provider_skills_from_different_dirs() {
    ...
    assert!(
        ...
        "should retain same-named skill in home directory, actual={result:?}"
    );
    assert!(
        ...
        "should retain same-named skill in project directory, actual={result:?}"
    );
}
...
fn test_unique_skills_name_dedup_same_name_different_providers() {
    ...
    assert_eq!(
        result.len(),
        1,
        "same name, different content, different provider should name-dedup; only retain highest priority provider"
    );
    assert_eq!(
        result[0].provider,
        SkillProvider::Agents,
        "name-dedup should retain high priority provider (Agents > Claude)"
    );
}
