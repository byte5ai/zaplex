//! Tool call readiness classification before BYOP request transmission.
//!
//! This module only handles projected-out safety metadata; it does not read raw prompts,
//! tool parameters, or tool output, nor does it modify controller, serializer, or conversation state.
...
pub const BLOCKED_BYOP_REQUEST_MESSAGE: &str =
    "Can't continue this conversation: an earlier tool result is missing or corrupted in this conversation's history, so Zaplex can't safely send the request to your provider. Start a new conversation or fork from an earlier point to continue.";
...
pub enum RepairSource {
    ...
    /// repair source recorded by `byop_fork_repair_state_json` for discarded tool results.
    ForkedHistory,
    /// Repair source recorded during recovery for tool result gaps written by older clients
    /// but unreplayable by current protocol. See `specs/byop-placeholder-tool-results/ISSUES.md`
    /// BYOP-PR-6: currently not produced in the recovery path; unexplained gaps in old BYOP
    /// conversations are handled as corrupted history blocks by default.
    /// **Do not casually delete this variant**: persisted sidecars may deserialize
    /// `restored_legacy_history`; deletion breaks sidecar format compatibility.
...
impl<'a> Classifier<'a> {
    ...
    fn handle_tool_result(&mut self, result: ProjectedToolResult) -> Option<ReadinessState> {
        // Split the check "does this belong to active group?" and "modify active_call state" into two steps:
        // filter with immutable borrow first, fall through to unattached path on failure,
        // then take mutable borrow only after passing, avoiding expect/unwrap in production paths.
    ...
    fn mark_stale_repair_records_for_visible_gap(&mut self, key: &ToolCallKey) {
        // Repair record authorization semantics require exact equality across three fields:
        // task_id + assistant_tool_call_message_id + tool_call_id; but when a real visible gap appears
        // (i.e., the same `tool_call_id` is explicitly marked missing in current conversation history),
        // any repair record with "same tool_call_id but different (task_id, assistant_message_id)"
        // no longer applies; mark as stale so the upper layer can signal in diagnostics that the record was ignored.
