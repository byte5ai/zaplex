...
/// An MCP server integration that Zaplex ships with bundled skills for.
...
impl TemplatableMCPServerManager {
    ...
    fn spawn_server_impl(
        ...
        if let TransportType::CLIServer(cli_server) = &mut server.transport_type {
            ...
            // continue to inherit Zaplex's process cwd.
...
async fn spawn_server(
    ...
    let service = match transport_type {
        TransportType::CLIServer(cli_server) => {
            ...
            // rmcp::transport::TokioChildProcess::builder requires tokio::process::Command,
            // cannot substitute with command::r#async::Command wrapper; use manual CREATE_NO_WINDOW
            // setting below to keep Windows from flashing console window.
            ...
            .map_err(|err| {
                if err.kind() == std::io::ErrorKind::NotFound {
                    let cwd_display = cwd_for_log
                        .as_deref()
                        .unwrap_or("<inherited from Zaplex's process cwd>");
                    logger.log(format!(
