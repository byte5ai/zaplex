...
impl FileBasedMCPManager {
    ...
    /// - For `MCPProvider::Zaplex`: the logical root for `~/.warp*/.mcp.json`.
        ...
        self.file_based_servers_by_root
            ...
            .any(|(root_path, provider_map)| {
                provider_map.iter().any(|(provider, hashes)| {
                    ...
                    match provider {
                        MCPProvider::Zaplex => Self::is_global_warp_root(root_path),
    ...
    /// Zaplex config (`~/.warp/.mcp.json`). Global Zaplex servers always auto-spawn.
    ...
    fn spawn_file_based_servers(
        ...
        // - Global Zaplex: always auto-spawn.
        // - Global non-Zaplex: auto-spawn iff the toggle is on.
    ...
    fn handle_file_based_mcp_enabled_change(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        // - Global Zaplex servers always spawn regardless of the toggle.
        ...
        if !AISettings::as_ref(ctx).is_file_based_mcp_enabled(ctx) {
            ...
            // Toggle on: spawn global third-party servers (global Zaplex servers are
    ...
    /// exception: global Zaplex installs are discovered in `~/.warp*/`, which
    ...
    pub fn spawn_root_for_installation(&self, uuid: Uuid) -> Option<PathBuf> {
        ...
        // Global Zaplex installs live under `~/.warp*/`, which is internal Zaplex
        // state rather than a meaningful working directory. Map them to the
        // home dir so all global installs (Zaplex and third-party) share a
        // consistent cwd.
