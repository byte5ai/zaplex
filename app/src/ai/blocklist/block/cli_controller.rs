    ...
    pub fn new(
        ...
        ctx.subscribe_to_model(action_model, |me, event, ctx| match event {
            ...
            BlocklistAIActionEvent::FinishedAction { action_id, .. } => {
                ...
                // causing the input box to disappear. Simultaneously, the `controller.rs:1112-1119`
                // branch reads `subagent_task_id`; if we don't actually create a conversation task,
                // the next query routing fails with "Could not find conversation for response stream".
                //
                // Solution uses the silent path: `create_silent_cli_subagent_task_for_conversation`
                // actually creates a subtask but doesn't emit `CreatedSubtask` (the original method's
                // emit timing is caller-controlled), letting this hook manually emit after block upgrade.
                // This walks the complete upstream upgrade chain and creates the SubagentView overlay,
                // and combined with `cli.rs:431-446` root_task fallback, when task has no exchange,
                // the view uses root last_exchange as placeholder during creation; later, when the user's
                // follow-up query routes to the subtask and triggers `AppendedExchange`, the view's own
                // subscription (`cli.rs:355-399`) automatically replaces the model with the real exchange.
                //
                // Side effect: the overlay briefly renders with root last_exchange at creation time,
                // which may briefly flash root content for one frame; but after
                // `set_agent_interaction_mode_for_agent_monitored_command` upgrade, SubagentView's
                // filtering only shows task_id-associated content, matching upstream experience.
                ...
                //
                // Note: terminal_model lock is already dropped above to avoid deadlock reproduced earlier
                // (emit SpawnedSubagent → terminal_view::handle_cli_subagent_controller_event
                // → CLISubagentView::new might synchronously lock terminal_model again, causing
                // FairMutex re-entrancy).
