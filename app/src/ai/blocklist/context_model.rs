...
/// Hard cap on inlining a single text-like PendingFile into the prompt;
/// exceeding this skips it (avoids bloating context). Differs from
/// `attachment_utils::MAX_ATTACHMENT_SIZE_BYTES` (10 MB, for binary attachments):
...
/// so we supplement with filename/extension matching.
...
/// Fallback filename/extension matching to cover extensionless conventional files
/// (Dockerfile, Makefile, .env, etc.).
...
pub struct BlocklistAIContextModel {
    ...
    /// Context attachments displayed in the input box with visible @ references.
...
impl BlocklistAIContextModel {
    ...
    pub fn pending_context(&self, app: &AppContext, is_user_query: bool) -> Vec<AIAgentContext> {
        ...
        let project_rules = if let Some(pwd) = pwd.clone().and_then(|path| {
            ...
            // Prefer the normal path (zero I/O; fetch from HashMap after async indexing).
            // When not ready, sync fast-path stat + read rule files in cwd/ancestor directories.
            // Align with opencode `findUp` semantics, ensuring cd + immediate query finds AGENTS.md.
            // Fast-path has internal caching + time budget; UI never blocks. See
    ...
    /// Register a context attachment that can be referenced by @ name in future queries.
    ...
    /// Return @ context attachments indexed by their visible reference strings.
    ...
    /// Return @ context attachments still present in the current query.
    ...
    /// Delete @ context attachments no longer present in the input box.
