//! "Reasoning Depth" picker in the bottom-right of the input box (BYOP mode).
//!
//! Follows the same template as `EnvironmentSelector`, simplified to:
//! - Data source: `LLMPreferences::get_reasoning_effort(...)` + variants table for the currently selected model
//! - State: `LLMPreferences::reasoning_effort_per_terminal` (session-only)
//! - No settings.toml writes, no telemetry, no cloud integration
//! - If the current model doesn't support reasoning (variants is empty) → entire component renders empty, picker naturally disappears
...
/// "Reasoning Depth" selector in the input box toolbar.
pub struct ReasoningDepthSelector {
    ...
    /// The (api_type, model_id) currently associated with this picker; refreshed when LLMPreferences changes.
...
impl ReasoningDepthSelector {
    pub fn new(
        ...
        let dropdown = ctx.add_typed_action_view(move |ctx| {
            ...
            // Cannot use Environments — its sidecar path would feed action_data() to
            // `ServerId::from_string_lossy`, and our effort names (e.g., "Auto"/"Off")
            // are not 22 characters, causing panic in debug builds.
            DisplayChipMenu::new(
    ...
    /// Parse the currently selected model → if it's a BYOP model, extract (api_type, model_id), otherwise None.
    ...
    fn refresh(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        // Variants is empty → entire component renders empty in subsequent render
        ...
        // Refresh button label / disabled state
    ...
    /// Whether the component should currently be rendered (model supports reasoning).
...
impl View for ReasoningDepthSelector {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        if !self.is_visible() {
            // Model doesn't support reasoning → render empty
