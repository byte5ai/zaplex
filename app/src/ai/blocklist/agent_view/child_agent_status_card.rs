...
impl View for ChildAgentStatusCard {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        for child in &children {
            ...
            // T3-7: an in-progress child agent surfaces its current action in the title,
            // e.g. "Untitled - ↳ Searching codebase..." / "Refactor X - ↳ Reading files...".
            // A finished child agent keeps showing its original title unchanged.
...
/// T3-7: extracts the current/most-recent action presence summary of a child
/// agent conversation, used to surface a subtitle like "↳ Searching codebase..."
/// on the parent ChildAgentStatusCard.
/// Mirrors the look of opencode's TUI Task parent card `↳ Bash $ git status`.
///
/// Implementation: iterate latest_exchange.output_status.output().messages in
/// reverse to find the last `AIAgentOutputMessageType::Action`, then take its
/// `presence_continuous_summary`. Returns None if none is found (the card shows no subtitle).
