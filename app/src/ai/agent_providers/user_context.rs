//! Render the attachment context (`AIAgentContext`) attached to a single `AIAgentInput::UserQuery`
//! into user message content sent to the upstream model (text prefix + binary multimodal parts).
//!
//! ## Alignment with warp's own protocol
//!
//! Under warp's native protocol, these attachments flow through `api::InputContext` fields
//! `executed_shell_commands / selected_text / files / images` (see `app/src/ai/agent/api/convert_to.rs`
//! `convert_context`). BYOP integrates directly with OpenAI / Anthropic / Gemini / Ollama-compatible
//! `/chat/completions` endpoints, which lack an `InputContext` layer, so data must be embedded
//! directly into the user message.
//!
//! Fields strictly align with warp protobuf and introduce no out-of-protocol fields:
//!
...
//!
//! - These attachments are injected only into the user message, not into the system prompt
//! - Previous rounds' `referenced_attachments` are re-rendered from persisted messages to avoid losing context across BYOP turns
//! - Environment-type context such as env / git / skills / project_rules / codebase / current_time
//!   continue to be rendered into system by `prompt_renderer`, without overlap with this module
...
/// Dual-channel result returned by `collect_user_attachments`.
///
/// - `prefix`: text prefix block to prepend to user message text. Contains inline XML for block / selected_text /
///   text-like files, plus placeholder hints for binary attachments (enabling LLM to reference filenames).
/// - `binaries`: attachments to inject as `ContentPart::Binary` into multimodal messages
///   (image / PDF / audio). The caller (chat_stream.rs) filters by model capability and decides
///   whether to switch to `MessageContent::Parts`.
...
/// A single binary attachment, equivalent to the input triple for genai `Binary::from_base64`.
#[derive(Debug, Clone)]
...
/// Render a single UserQuery's attachment context as "text prefix + binary parts".
///
/// Callers should:
/// 1. Prepend `prefix` before the user query text (leaving a blank line in between)
/// 2. Filter `binaries` by model capability and switch to `MessageContent::Parts` if any remain
pub fn collect_user_attachments(ctx: &[AIAgentContext]) -> UserAttachments {
    ...
    for c in ctx {
        match c {
            ...
            // Environment-type context is handled by prompt_renderer and does not enter user message.
    ...
    // ----- binaries (for caller to filter by capability and inject as ContentPart::Binary) -----
    for img in &images {
        out.binaries.push(UserBinary {
            name: img.file_name.clone(),
            content_type: img.mime_type.clone(),
            // ImageContext.data is already a base64 string (encoding is completed by
            // `read_and_process_images_async` in the sibling path `process_non_image_files` when PendingAttachment::Image is enqueued)
            data: img.data.to_string(),
        });
    }
    for f in &binary_files {
        if let AnyFileContent::BinaryContent(bytes) = &f.content {
            // Empty BinaryContent indicates "upstream intentionally did not read bytes" (.exe / very large file / non-multimodal
            // mime), so only the placeholder XML path is used, not ContentPart::Binary.
            if bytes.is_empty() {
                continue;
            }
            let b64 = base64::engine::general_purpose::STANDARD.encode(bytes);
            // Guess mime type from the extension in file_name; `mime_guess` uses the same rules as
            // process_non_image_files, and we recalculate here because FileContext does not store mime.
...
/// Backward compatibility for old callers: only returns prefix text. New code should use `collect_user_attachments`.
#[cfg(test)]
...
/// Binary file prefix placeholder: informs the LLM of the file's existence, complete path, and mime type,
/// allowing it to decide:
/// - Whether to read ContentPart::Binary directly (model supports it + bytes already uploaded)
/// - Or use read_files / shell tools to handle by path (.exe / .zip / very large files)
///
/// Actual bytes flow through caller's `MessageContent::Parts` as ContentPart::Binary;
/// we do **not** duplicate base64 here (avoiding doubled token count + slowness on long base64 in many models).
fn render_file_binary_placeholder(out: &mut String, f: &FileContext) {
    use std::fmt::Write;
    let path = xml_attr(&f.file_name);
    // mime is inferred by mime_guess from filename/extension, consistent with process_non_image_files.
    let mime = mime_guess::from_path(&f.file_name)
        .first_or_octet_stream()
        .to_string();
    let size = match &f.content {
        AnyFileContent::BinaryContent(bytes) if !bytes.is_empty() => Some(bytes.len()),
        // bytes empty (very large file / non-multimodal binary) → size unknown, don't output size attribute,
        // let AI query with read_files / Get-Item or similar.
        _ => None,
    };
...
/// Image prefix placeholder: same semantics as binary file, actual data flows through ContentPart::Binary into multimodal.
