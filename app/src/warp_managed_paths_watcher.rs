...
/// Duration between filesystem watch events for the Zaplex managed paths watcher, in milliseconds.
#[cfg(not(target_family = "wasm"))]
const ZAPLEX_MANAGED_PATHS_WATCHER_DEBOUNCE_MILLI_SECS: u64 = 500;
...
pub(crate) fn ensure_warp_watch_roots_exist() {
    ...
    if let Err(err) = fs::create_dir_all(&data_dir) {
        log::warn!(
            "Failed to create Zaplex data directory {}: {err}",
            data_dir.display()
        );
    }
    ...
    if config_local_dir != data_dir {
        if let Err(err) = fs::create_dir_all(&config_local_dir) {
            log::warn!(
                "Failed to create Zaplex config directory {}: {err}",
        ...
        let watcher = if should_register_watcher {
            ctx.add_model(|ctx| {
                BulkFilesystemWatcher::new(
                    Duration::from_millis(ZAPLEX_MANAGED_PATHS_WATCHER_DEBOUNCE_MILLI_SECS),
                    ctx,
                )
            })
        } else {
                ...
                (warp_home_config_dir(), warp_home_mcp_config_file_path())
            {
                        ...
                        || !warp_home_config_dir.starts_with(&config_local_dir))
                {
                    Self::register_path(
                        ...
                        "Zaplex home MCP config directory",
                    );
                }
            }
        }
...
mod tests {
    ...
    fn warp_managed_mcp_config_path_contains_only_warp_home_path() {
        match (
            ...
            (_, _, None) => {}
            _ => panic!("Expected Zaplex MCP path when home directory is available"),
        }
    }
