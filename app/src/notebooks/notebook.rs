...
use crate::{
    ...
    drive::{
        ...
        export::ExportManager, items::WarpDriveItemId, ObjectTypeAndId, ZaplexDriveObjectSettings,
    },
    editor::{
...
/// A view that allows viewing/execution and editing of a Zaplex notebook.
...
impl NotebookView {
    ...
    fn handle_active_notebook_event(
        ...
        match event {
            ...
            ActiveNotebookDataEvent::CreatedOnServer => {
                ...
                {
                    // TODO(zap-cloud-removal Phase 5): same as above, sharing UI is retired,
                    // notebook ShareableObject injection removed; clean up id flow when cloud_object is retired.
                    let _ = id;
                }
            }
    ...
    pub fn load(
        ...
        // TODO(zap-cloud-removal Phase 5): sharing UI is retired, notebook
        // ShareableObject injection removed; cloud_object server_id path kept until Phase 5.
        let _ = notebook.id;
        ...
        self.update_breadcrumbs(ctx);
        // Zaplex Phase 2a: invitee-driven sharing dialog removed.
    ...
    fn conflict_dialog_refresh_button_clicked(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if let Some(notebook) = ObjectStoreModel::as_ref(ctx).get_notebook(&id) {
            self.load(notebook.clone(), &ZaplexDriveObjectSettings::default(), ctx);
        }
        ctx.notify();
    }
