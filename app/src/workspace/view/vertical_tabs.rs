...
enum SummaryPaneKind {
    ...
    Settings,
    EnvVarCollection,
    // Zap Wave 7-3: the `EnvironmentManagement` variant was physically deleted along with the ambient-agent UI subsystem.
    AIFact,
    AIDocument,
...
fn resolve_icon_with_status_variant(
    ...
    match typed {
        ...
        // Zap Wave 7-3: `TypedPane::EnvironmentManagement` was physically deleted along with the ambient-agent UI subsystem.
...
impl TypedPane<'_> {
    fn summary_pane_kind(&self, title: &str, app: &AppContext) -> SummaryPaneKind {
        match self {
            ...
            // Zap Wave 7-3: the `TypedPane::EnvironmentManagement` arm was physically deleted along with the variant.
    ...
    fn kind_label(&self) -> String {
        match self {
            ...
            // Zap Wave 7-3: the `TypedPane::EnvironmentManagement` kind_label arm was physically
    ...
    fn icon(&self) -> WarpIcon {
        match self {
            ...
            // Zap Wave 7-3: the `TypedPane::EnvironmentManagement` icon arm was physically deleted along with the variant.
...
impl PaneGroup {
    fn resolve_pane_type(&self, pane_id: PaneId, app: &AppContext) -> TypedPane<'_> {
        match pane_id.pane_type() {
            ...
            // Zap Wave 7-3: the `EnvironmentManagement` arm was physically deleted together with the variant.
...
fn render_summary_pane_kind_icon_circle(
    ...
    let (icon_element, background): (Box<dyn Element>, ElementFill) = match kind {
        ...
        // Zap Wave 7-3: the `SummaryPaneKind::EnvironmentManagement` arm was physically deleted along with the variant.
