...
enum SummaryPaneKind {
    ...
    Settings,
    EnvVarCollection,
    // Zaplex Wave 7-3: the `EnvironmentManagement` variant was physically deleted along with the ambient-agent UI subsystem.
    AIFact,
    AIDocument,
...
fn resolve_icon_with_status_variant(
    ...
    match typed {
        ...
        // Zaplex Wave 7-3: `TypedPane::EnvironmentManagement` was physically deleted along with the ambient-agent UI subsystem.
        TypedPane::Settings => IconWithStatusVariant::Neutral {
            icon: typed.icon(),
            icon_color: main_text,
        },
        // Zaplex Drive object types use their established index colors
...
impl TypedPane<'_> {
    fn summary_pane_kind(&self, title: &str, app: &AppContext) -> SummaryPaneKind {
        match self {
            ...
            // Zaplex Wave 7-3: the `TypedPane::EnvironmentManagement` arm was physically deleted along with the variant.
    ...
    fn kind_label(&self) -> String {
        match self {
            ...
            // Zaplex Wave 7-3: the `TypedPane::EnvironmentManagement` kind_label arm was physically
            // deleted along with the variant.
    ...
    fn icon(&self) -> WarpIcon {
        match self {
            ...
            // Zaplex Wave 7-3: the `TypedPane::EnvironmentManagement` icon arm was physically deleted along with the variant.
...
impl PaneGroup {
    fn resolve_pane_type(&self, pane_id: PaneId, app: &AppContext) -> TypedPane<'_> {
        match pane_id.pane_type() {
            ...
            // Zaplex Wave 7-3: the `EnvironmentManagement` arm was physically deleted together with the variant.
...
fn render_summary_pane_kind_icon_circle(
    ...
    let (icon_element, background): (Box<dyn Element>, ElementFill) = match kind {
        ...
        // Zaplex Wave 7-3: the `SummaryPaneKind::EnvironmentManagement` arm was physically deleted along with the variant.
