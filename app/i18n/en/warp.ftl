...
settings-ai-aws-bedrock-description = Zaplex loads and sends local AWS CLI credentials for Bedrock-supported models.
settings-ai-aws-bedrock-description-managed = Zaplex loads and sends local AWS CLI credentials for Bedrock-supported models. This setting is managed by your organization.
...
settings-agent-providers-description = Zaplex detects your installed Claude Code and Codex CLIs and runs them as their own agents — no provider setup or API keys needed. Adding a provider here is optional: it lets the in-app agent connect to a compatible model endpoint that you configure manually. Provider metadata is stored in the local settings.toml; API keys are stored securely in the system keychain.
...
settings-agent-providers-loading-catalog = Loading…
settings-agent-providers-catalog-empty = No entries to show.
...
settings-agent-providers-name-placeholder = Provider name (e.g. My Provider)
settings-agent-providers-api-key-placeholder = API key (optional; leave empty if not required)
...
settings-agent-providers-model-name-placeholder = Display name (e.g. My Model)
settings-agent-providers-model-id-placeholder = Model ID (the `model` field sent to the API, e.g. my-model)
...
settings-agent-providers-sync-models-dev = Sync
...
workflow-ai-assist-tooltip = Generate a title, descriptions, or parameters with Zaplex AI
workflow-tooltip-restore-from-trash = Restore workflow from trash
workflow-ai-assist-error-byop-required = Autofill requires a BYOP model. Configure a provider and model in Settings → Agents → Providers.
