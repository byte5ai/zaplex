...
cockpit-spawn-card-no-cli = No agent CLI installed — install Claude Code or Codex
...
fm-toast-server-config-read-failed = Failed to read server configuration: { $err }
...
fm-toast-symlink-target-unresolved = Link target could not be resolved: { $err }
