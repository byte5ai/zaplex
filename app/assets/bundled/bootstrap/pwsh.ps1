...
$null = New-Module -Name Warp-Module -ScriptBlock {
    ...
    function Warp-Bootstrapped {
        [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSUseDeclaredVarsMoreThanAssignments', 'ZAPLEX_BOOTSTRAPPED', Justification = 'False positive as we are assigning to global')]
    ...
    function Warp-Precmd {
        ...
        if ($script:generatorCommand -eq $true) {
            ...
            if ($global:ZAPLEX_BOOTSTRAPPED -eq 1) {
            ...
            $honor_ps1 = "$env:ZAPLEX_HONOR_PS1" -eq '1'
    ...
    if ((Test-Path env:ZAPLEX_INITIAL_WORKING_DIR) -and -not [String]::IsNullOrEmpty($env:ZAPLEX_INITIAL_WORKING_DIR)) {
        Set-Location $env:ZAPLEX_INITIAL_WORKING_DIR 2> $null
        Remove-Item -Path env:ZAPLEX_INITIAL_WORKING_DIR
    }
...
. {
    ...
    if (-not [String]::IsNullOrEmpty($env:ZAPLEX_PATH_APPEND)) {
        $env:PATH = '{0}{1}{2}' -f $env:PATH, [IO.Path]::PathSeparator, $env:ZAPLEX_PATH_APPEND
