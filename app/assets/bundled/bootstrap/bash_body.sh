...
if [ -z "$ZAPLEX_BOOTSTRAPPED" ]; then
    ...
    warp_send_json_message () {
        ...
        # Note that $ZAPLEX_USING_WINDOWS_CON_PTY is set in the init shell script.
    ...
    warp_preexec () {
        ...
        if [[ "$BASH_COMMAND" != warp_run_generator_command* ]] && [[ -f $_ZAPLEX_GENERATOR_PIDS_STARTED_TMP_FILE ]] && [[ -f $_ZAPLEX_GENERATOR_PIDS_COMPLETED_TMP_FILE ]]
        then
          ...
          while IFS= read -r pid; do
            completed_pids+=("$pid")
          done < $_ZAPLEX_GENERATOR_PIDS_COMPLETED_TMP_FILE
          ...
          done < $_ZAPLEX_GENERATOR_PIDS_STARTED_TMP_FILE
    ...
    # Users can disable the auto title if they chose to by setting ZAPLEX_DISABLE_AUTO_TITLE.
    warp_title () {
    ...
    warp_set_title_idle_on_precmd () {
      # If the user wants to set the title themselves, they can set the ZAPLEX_DISABLE_AUTO_TITLE flag.
      if [ ! -z "$ZAPLEX_DISABLE_AUTO_TITLE" ]; then
        return
      fi
    ...
    warp_precmd () {
        ...
        if [ "$ZAPLEX_IN_MSYS2" = true ]; then
          ...
          warp_send_hook_kv_pair "next_block_id" "precmd-$ZAPLEX_SESSION_ID-$((block_id++))"
          warp_send_hook_via_kv_pairs_end
        else
        ...
        # are run.
        if [ ! -z  $_ZAPLEX_GENERATOR_COMMAND ]; then
            ...
            unset _ZAPLEX_GENERATOR_COMMAND
        ...
        if [[ -z $ZAPLEX_INPUT_REPORTING_SUPPORTED ]]; then
          ZAPLEX_INPUT_REPORTING_SUPPORTED=$(warp_input_reporting_supported)
        fi

        # If we haven't already, cache information about supported features.
        if [[ -z $ZAPLEX_PS1_EXPANSION_SUPPORTED ]]; then
          ZAPLEX_PS1_EXPANSION_SUPPORTED=$(warp_ps1_expanding_supported)
        fi

        if [[ $ZAPLEX_PS1_EXPANSION_SUPPORTED  == "1" ]]; then
          ...
          deref_ps1=$(echo -e "\n" | PS1="$ZAPLEX_PS1" BASH_SILENCE_DEPRECATION_WARNING=1 "$BASH" --norc -i 2>&1 | command -p head -2 | command -p tail -1)
        fi

        # Escaped PS1 variable
        local escaped_ps1
        if [ "$ZAPLEX_IN_MSYS2" = false ]; then
        ...
        if [[ $ZAPLEX_INPUT_REPORTING_SUPPORTED == "1" ]]; then
          bind -r '"\ei"'
        ...
        if [[ -n "$ZAPLEX_BOOTSTRAPPED" ]]; then
          if [[ -n "$VIRTUAL_ENV" ]] && [ "$ZAPLEX_IN_MSYS2" = false ]; then
              escaped_virtual_env=$(warp_escape_json "$VIRTUAL_ENV")
          fi

          if [[ -n "$CONDA_DEFAULT_ENV" ]] && [ "$ZAPLEX_IN_MSYS2" = false ]; then
          ...
          if command -v node > /dev/null 2>&1 && [ "$ZAPLEX_IN_MSYS2" = false ]; then
        ...
        # parse error.
        # Note ZAPLEX_SESSION_ID doesn't need to be escaped since it's a number
        ...
        if [ "$ZAPLEX_IN_MSYS2" = true ]; then
          ...
          warp_send_hook_kv_pair "session_id" "$ZAPLEX_SESSION_ID"
          warp_send_hook_via_kv_pairs_end
        else
    ...
    # Changes the ZAPLEX_HONOR_PS1 variable to 1, to indicate we want to use the PS1. Restores
    ...
    # Changes the ZAPLEX_HONOR_PS1 variable to 0, to indicate we want to use the Warp prompt. Calls 
...
TMPPREFIX="'$HOME/.zshtmp-'" ZAPLEX_SSH_RCFILES="'${ZDOTDIR:-$HOME}'" ZDOTDIR="'$ZAPLEX_TMP_DIR'" exec -l zsh -g $TRACE_FLAG_IF_ZAPLEX_SHELL_DEBUG_MODE
      ;;
esac
"
        }
    ...
    #
    #
    if [[ $HISTFILESIZE == $ZAPLEX_INITIAL_HISTFILESIZE ]]; then
        unset HISTFILESIZE
    fi
    unset ZAPLEX_INITIAL_HISTFILESIZE
