...
setopt hist_ignore_space
 ...
 _msg=$(printf "{\"hook\": \"InitShell\", \"value\": {\"session_id\": $ZAPLEX_SESSION_ID, \"shell\": \"zsh\", \"user\": \"%s\", \"hostname\": \"%s\", \"is_subshell\": true, \"wsl_name\": \"$WSL_DISTRO_NAME\"}}" "$_user" "$_hostname" | command -p od -An -v -tx1 | command -p tr -d " \n")
 ZAPLEX_USING_WINDOWS_CON_PTY=@@USING_CON_PTY_BOOLEAN@@
 if [ "$ZAPLEX_USING_WINDOWS_CON_PTY" = true ]; then printf '\e]9278;d;%s\x07' "$_msg"; else printf '\e\x50\x24\x64%s\x9c' "$_msg"; fi
