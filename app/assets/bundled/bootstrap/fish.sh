...
if test -n "$ZAPLEX_INITIAL_WORKING_DIR"
    cd "$ZAPLEX_INITIAL_WORKING_DIR" >/dev/null 2>&1
    set -e ZAPLEX_INITIAL_WORKING_DIR
end

# Append additional PATH entries if provided via ZAPLEX_PATH_APPEND.
if test -n "$ZAPLEX_PATH_APPEND"
    set -gx --path PATH "$PATH:$ZAPLEX_PATH_APPEND"
    set -e ZAPLEX_PATH_APPEND
end
...
function warp_send_json_message
    ...
    if [ "$ZAPLEX_USING_WINDOWS_CON_PTY" = true ]
...
function  _warp_run_generator_command_internal
    ...
    fish -c "
        set -l warp_using_windows_con_pty $ZAPLEX_USING_WINDOWS_CON_PTY;
...
function warp_run_generator_command
    ...
    set -g _ZAPLEX_GENERATOR_COMMAND 1
...
function warp_update_prompt_vars
  ...
  # If honoring PS1, add prefix/suffix to both prompts
    ...
    function end_prompt        
      echo -n (printf '\x1b')
      if test "$ZAPLEX_HONOR_PS1" != "1" && [ "$ZAPLEX_USING_WINDOWS_CON_PTY" = true ]
...
# Changes the ZAPLEX_HONOR_PS1 variable to 1, to indicate we want to use the user's custom prompt. Restores
...
# Changes the ZAPLEX_HONOR_PS1 variable to 0, to indicate we want to use the Warp prompt. Saves and clears
...
function warp_precmd --on-event fish_prompt --on-event fish_posterror
    ...
    if ! test -z $_ZAPLEX_GENERATOR_COMMAND
        set -e _ZAPLEX_GENERATOR_COMMAND
...
if test "$ZAPLEX_IS_LOCAL_SHELL_SESSION" = "1"
...
hook="'$(printf "{\"hook\": \"SSH\", \"value\": {\"socket_path\": \"'$SSH_SOCKET_DIR/$ZAPLEX_SESSION_ID'\", \"remote_shell\": \"%s\"}}" "${SHELL##*/}" | command od -An -v -tx1 | command tr -d " \n")'"
...
bash)
    ...
    ;;
zsh) ZAPLEX_TMP_DIR="'$(mktemp -d warptmp.XXXXXX)'"
local ZSH_ENV_SCRIPT='$zsh_env_script'
if [[ "'$?'" == 0 ]]; then
  if command -v xxd >/dev/null 2>&1; then
    echo "'$ZSH_ENV_SCRIPT'" | command xxd -p -r > "'$ZAPLEX_TMP_DIR'"/.zshenv
  else
...
"
    ...
    function ssh
        if is_interactive_ssh_session $argv
            ...
            if [ "$ZAPLEX_USE_SSH_WRAPPER" = "1" ]
                if test $ZAPLEX_SHELL_DEBUG_MODE
                    set -g TRACE_FLAG_IF_ZAPLEX_SHELL_DEBUG_MODE "-x"
                else
                    set -g TRACE_FLAG_IF_ZAPLEX_SHELL_DEBUG_MODE ""
                end
