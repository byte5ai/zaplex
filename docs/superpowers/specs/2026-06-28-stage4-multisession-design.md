...
## 4. Korrekturen für die Verbindungsliste (#455 / #456)

- Titel und Identität bleiben bei 250, 320 und 480 px nutzbar: Titel erhalten eine Ellipse, Metadaten dürfen umbrechen und die Öffnen-Aktion behält ihren Platz. Die anonymisierte [HTML-Layout-Fixture](../../ui/connections-session-rows.html) dokumentiert die Breitenaufteilung; sie ersetzt keine native UI-Abnahme.
- Eine laufende Aktualisierung zeigt vorhandene Sessions weiter mit einem Aktualisierungshinweis, solange kein Fehler vorliegt. Ein fehlgeschlagener Abruf zeigt den Fehler statt veralteter Zeilen; auch bei erneuten Abfragen bleibt dieser bis zur erfolgreichen Antwort sichtbar. Das gesamte Inventar einschließlich Vorbereitung, Warteschlange und historischen Daemons hat ein Zeitlimit von 30 Sekunden. Die Inventarabfrage installiert keinen Dienst: Falls eine Erstinstallation erforderlich ist, fordert sie zum Verbinden des Hosts auf. Der normale Verbindungsablauf behält seine Installation mit Fortschrittsanzeige.
- Auch frisch geöffnete Sessions werden mit ihrer bestätigten PTY-ID und Generation dem besitzenden Terminal zugeordnet. Ein Klick auf diese Session aktiviert den vorhandenen Tab und die Shell; ein darüber geöffneter Dateimanager wird geschlossen. Die Zuordnung bleibt nach Host und Daemon-Laufzeit getrennt.
- Eine abgewiesene oder fehlgeschlagene Verbindung vor dem Shell-Handshake beendet den Startzustand und macht die Initialisierungsausgabe sichtbar. Der erstmalige Verbindungsaufbau einschließlich Attach und Replay hat ein Zeitlimit von 60 Sekunden. Sobald der Attach abgeschlossen und der Shell-Protokollstart (`InitShell`) oder die vollständige Initialisierung bestätigt ist, ist dieses Startzeitlimit dauerhaft erledigt. Interaktive Abfragen aus Shell-Startdateien bleiben damit bedienbar, auch wenn sie die vollständige Initialisierung verzögern; spätere Wiederverbindungen werden ebenfalls nicht vom Startzeitlimit unterbrochen.
- Ein lokaler Abbruch meldet ausschließlich den eigenen Transport ab. Er beendet weder die Remote-PTY noch den gemeinsamen SSH-ControlMaster. Verspätete Antworten dürfen einen abgebrochenen Tab nicht erneut verbinden.
